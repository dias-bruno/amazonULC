...
For the other three study areas, we made the delimitation according to the methodology [@Goncalves2021] - which uses night light images to identify potentially populated areas.
...
The metrics F1-Score Macro (average of the F1-Score of the different classes), F1-Score Weighted (average of the F1-Score of the different classes weighted by the number of samples), and Global Accuracy of the classification produced by the RF model over the validation samples are presented in Table 2 for each study area.
...
![Urban land cover classification maps of Altamira, Cametá, Belém Metropolitan Area, Marabá and Santarém.](images/ULC_Legend_v2-tile.jpg){width="4724"}
...
This work seeks to provide land cover bases for five locations in the Brazilian Legal Amazon.
However, although it has met its objective, some limitations must be pointed out.
...
It is essential to mention that this limitation is due to the research team's decision to use only algorithms available in open-access software.
